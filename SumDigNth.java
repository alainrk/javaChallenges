...
From: https://www.codewars.com/kata/55ffb44050558fdb200000a4/train/java
...
public class SumDigNth {
    ...
    public static long sumDigNthTerm(long initval, long[] patternl, int nthterm) {
        ...
        while (c<nthterm) {
          initval += patternl[i];
          i = (i+1) % patternl.length;
          c++;
        }

        String r = Long.toString(initval);
        for (int j=0; j<r.length(); sum += Long.parseLong(Character.toString(r.charAt(j++))));
        return sum;
      }
}
